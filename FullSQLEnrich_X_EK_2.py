...
class EnrichFullSQLES:
  ...
  def __init__(self,param_json):
    ...
    print (self.ESServer, " / ", self.ESUser, " / " , self.ESPwd )
    try :
       self.es = Elasticsearch([self.ESServer], http_auth=(self.ESUser, self.ESPwd))
       # self.es = Elasticsearch([self.ESServer], basic_auth=(self.ESUser, self.ESPwd))
...
# --- Metadata  ----
  def MetaData(self):
    ...
    A_DB_USERS_tmp = p1.es.search(index="a_db_users", body={"query": {"match_all": {}}, "size": 10000})
    # print("Got %d Hits:" % A_IPs_tmp['hits']['total']['value'])
    # print ("Type : ", type(A_IPs_tmp))
    ...
    A_SEL_TYP_tmp = p1.es.search(index="a_sel_typ", body={"query": {"match_all": {}}, "size": 10000})
    # print("Got %d Hits:" % A_IPs_tmp['hits']['total']['value'])
    # print ("Type : ", type(A_IPs_tmp))
  ...
  def enrich_by_metadata(self,line):
    ...
    line = p1.enrich_server(line)
    line = p1.enrich_client(line)
    line = p1.enrich_DB_User(line)
    line = p1.enrich_Sel_Type(line)
    line = p1.enrich_misc(line)
    line = p1.confidence_level(line)
    return(line)
  ...
  def process_all_files(self,DataFiles):
    ...
    for datafile in DataFiles:
        ...
        # -- MAIN PROCESSING
        p1.fullSQLMany=[]
        doc_count=p1.process_one_file(datafile)
        print("Nbr of Docs to be Inserted", doc_count)
        if doc_count > 0 :
           p1.insert_many_Elastic()

        os.system('rm -f ' + p1.InProg)

        p1.rename_file(datafile)
  ...
  def process_one_line(self,line):
         ...
         # print(lineDict)
         line_meta = p1.enrich_one_line(lineDict)
  ...
  def enrich_one_line(self,lineDict):
      ...
      line_meta = p1.enrich_by_metadata(lineDict)
...
if __name__ == '__main__':
    ...
    if len(DataFiles) == 0 :
       print ("NO file to process for Coll : " , p1.collector)
       os.system('rm -f ' + p1.InProg)
       exit(0)
